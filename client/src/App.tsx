...
function AppRouter() {
  ...
  useEffect(() => {
    ...
    if (
      !user.hasOnboarded &&
      location.startsWith("/launch") &&
      location !== "/launch/onboarding" &&
      location !== "/launch/login"
    ) {
  ...
  // If not authenticated, show redirect indicator
...
function Router() {
  ...
  if (isNative && location === "/") {
    return <div className="min-h-screen w-full bg-[#0c0c1d]" />;
  }
